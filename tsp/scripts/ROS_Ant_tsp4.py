...
class ROS_tsp:
	def __init__(self):
		...
		self.multi = rospy.Subscriber("/stop/tsp", String, self.callback)
		self.multi = rospy.Subscriber("/tf", PoseStamped, self.sub_robot_tf)
	...
	def sub_robot_tf(self, robot_tf):
		"""
		self.tf_x = robot_tf.pose.pose.position.x 
		self.tf_y = robot_tf.pose.pose.position.y
		self.tf_orientation_x = robot_tf.pose.pose.orientation.x
		self.tf_orientation_y = robot_tf.pose.pose.orientation.y
		self.tf_orientation_z = robot_tf.pose.pose.orientation.z
		self.tf_orientation_w = robot_tf.pose.pose.orientation.w
		"""

		self.tf_x = robot_tf.transforms.transform.translation.x 
		self.tf_y = robot_tf.transforms.transform.translation.y
			...
			if k == 0:
				...
				if __name__ == '__main__':
					try:
						...
						if (self.tf_x < self.x + 10 and self.tf_x > self.x - 10) and (self.tf_y < self.y + 10 and self.tf_y > self.y - 10):
